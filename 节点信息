 ...
 * - [custom_prefix] 地标前自定义前缀 默认: 【自定义】 (新增功能, 留空则不显示前缀)
 * 关于缓存时长
...
async function operator(proxies = [], targetPlatform, context) {
  ...
  const custom_prefix = $arguments.custom_prefix || ''; // 默认空字符串，需显示前缀时自行设置
  ...
  if (internal) {
    ...
    format = format || '{{api.countryCode}}';
    url = $arguments.api || 'http://checkip.amazonaws.com';
  } else {
    format = format || '{{api.country}}';
  }
  ...
  // 处理节点名：有效节点生成带自定义前缀的地标名，失效节点保留原始名
  const nameCount = {};
  proxies = proxies.map(p => {
    const originalName = p.name;
    if (p._geo && (p._geo.country || p._geo.countryCode)) {
      // 生成基础名称（处理国家和城市相同的情况）
      const baseName = generateBaseName(p._geo);
      // 仅当custom_prefix存在时添加前缀
      const prefixedName = custom_prefix ? `${custom_prefix}${baseName}` : baseName;
      // 处理重复名称序号（序号从1开始，第一个同名节点无后缀，第二个加1，第三个加2，依此类推）
      nameCount[prefixedName] = (nameCount[prefixedName] || 0) + 1;
      const suffix = nameCount[prefixedName] > 1 ? nameCount[prefixedName] : ''; // 第一个节点无后缀，后续递增（如第一个"香港"，第二个"香港2"，第三个"香港3"）
      p.name = `${prefixedName}${suffix}`;
    } else {
      // 失效节点恢复原始名称
      p.name = originalName;
    }
  ...
  function formatter({ proxy = {}, api = {}, format = '', regex = '' }) {
    ...
    // 核心命名逻辑：处理国家和城市关系
    const baseName = generateBaseName(api);
    return baseName; // 直接返回处理后的名称（前缀在map循环中根据custom_prefix判断是否添加）
  }

  function generateBaseName(api) {
    // 处理异常情况：城市=国家（如香港-香港）
    const country = api.country || '未知国家';
    const city = api.city || '未知城市';
    return city === country ? country : `${country}-${city}`; // 异常情况只显示国家，正常情况显示国家-城市
  }
